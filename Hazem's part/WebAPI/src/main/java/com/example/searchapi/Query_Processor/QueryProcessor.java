...
public class QueryProcessor {
    private final IndexerImpl indexer;
    private final Set<String> stopWords;
    private final PorterStemmer stemmer;

    public QueryProcessor(IndexerImpl indexer) {
    ...
    private List<String> preprocessQuery(String query, boolean isPhraseSearch) {
        ...
        // Convert to lowercase if it's no phrase search and split into terms
        String[] tokens = isPhraseSearch ? processed.split("\\s+") : processed.toLowerCase().split("\\s+");
        ...
        for (String token : tokens) {
            if (!token.isEmpty()) {
                if (isPhraseSearch) {
                    ...
                    System.out.println("Stemmed Token: " + stemmer.stem(token));
                }
